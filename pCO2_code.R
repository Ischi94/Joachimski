...
pCO2 <- tibble(Sz = seq(500, 1500, 100), 
  ...
  pivot_wider(names_from = temp_range, 
              values_from = empty_row, names_prefix = "temp_")
...
get_pCO2 <- function(d13C_atm_value) {
  for (i in seq_along(d13Cs)) {
    for (j in seq_along(pCO2$Sz)) {
      pCO2[i, j +1] <-
        pCO2$Sz[j] * ((d13Cs[i] - 1.0044 * sol_dat$d13Cresp[x] - 4.4) / 
                        (d13C_atm_value - d13Cs[i]))
    }
  }
  
  # convert to long format for proceeding
  pCO2 %>%
    pivot_longer(cols = -Sz, 
                 names_to = "soil_temp",
                 values_to = "pCO2")
}

pCO2_long <- d13C_atm_range %>% 
  map_dfr(get_pCO2)
